	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			var defaults = {
                    ...
                    onCompleteSubmit : function(){},                  //上传完提交表单的方法
			};
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '				</div>';
					html += '			</div>';
					html += '			<div id="preview" class="upload_preview gridly"></div>';//add by hf
					html += '		</div>';
		            ...
		            html += '				</div>';
		            html += '			</div>';
		            html += '			<div id="preview" class="upload_preview gridly">';
                ...
                self.gridlyDrag();
                $(self).find(".upload_append_list").each(function(k,v){
                    $("#preview").append(v);//添加进去
	            ...
	            // 初始化html之后绑定按钮的点击事件
	            this.addEvent();
                this.funDeleteUploaded();//添加删除已经上传的图片事件

				// 绑定删除按钮
				//self.createCorePlug.funBin;
				//funBindHoverEvent();

			};
            //删除
            this.funDeleteUploaded=function(){

                if($(".ufile_del").length>0){
                    // 绑定删除图片方法
                    $(".ufile_del").click(function() {
                      //  ZYFILE.funDeleteFile(parseInt($(this).attr("data-index")), true);
                        $("#uploadedList_" + $(this).attr("data-index")).remove();//移除
                        return false;
                    });
                }
            ...
            //add by hf  给所有的添加拖动排序属性
            this.gridlyDrag=function(){
                //添加 add by hf
                $('.gridly').gridly({
                    base: 30, // px
                    gutter: 20, // px
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
                        ...
                        //把图片名字修改后台保存实际图片的名字 修改名字
                        $("#uploadList_" + file.index).find(".file_name").first().html(response);
					},
                    ...
                    myFormSubmit: function(){//提交表单
						var names={};
						var orders={};
						//获取所有名字
						$("#preview").find(".upload_append_list .file_name").each(function(k,v){
							names[k]=v.innerHTML;
						});
						//遍历所有的图片进行排序
						$("#preview > .upload_append_list").each(function(k,v){
							var  order= parseInt(v.style.left)/150+ parseInt(v.style.top)/122*4;
							if(isNaN(order)){
								order=k;
							}
							orders[order]=names[k];//做好排序
						});
                        var upnames="";
						//把图片的顺序加入表单
						...
						upnames=upnames.substring(0,upnames.length-1);
                        var fileInput="<input type='hidden' name='"+para.inputName+"' value='"+upnames+"'/>";
                        //alert(fileInput);
                        $(self).append(fileInput);//添加组件
                        //然后执行外部提供的提交表单方法
                        this.onCompleteSubmit();
					},
				};
