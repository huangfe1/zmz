...
var ZYFILE = {
       ...
       fileInput : null,                // 选择文件按钮dom对象
        ...
        onCompleteSubmit:function(){                 //提供给外部完成后提交表单提交表单

        },
        submit:function(){                          //提供个外部调用的方法
            if(ZYFILE.funReturnNeedFiles().length > 0){
                this.funUploadFiles(true);//上传后提交
            }else{
               // this.onCompleteSubmit();//如果没有需要完成的 直接提交
                this.myFormSubmit();//直接提交
            }
		}
        ,
		...
		// 上传多个文件  上传完是否提交表单
		funUploadFiles : function(isSubmitForm){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			// 遍历所有文件  ，在调用单个文件上传的方法
			$.each(this.uploadFile, function(k, v){
				self.funUploadFile(v,isSubmitForm);
			});
		},
		// 上传单个个文件 上传完是否提交表单
		funUploadFile : function(file,isSubmitForm){
			var self = this;  // 在each中this指向没个v  所以先将this保留
		    ...
		    xhr.addEventListener("load", function(e){
		    	...
		    	if(self.uploadFile.length==0){
                    ...
                    if(isSubmitForm){//如果提交
                        self.myFormSubmit();//提交表单
                    }
		    	}
		    }, false);  
		    // 错误
			...
			xhr.open("POST",self.url, true);//立即返回  异步提交
			xhr.setRequestHeader("X_FILENAME", file.name);
			//xhr.setRequestHeader("Content-Type","multipart/form-data"); 这一行不需要,否则会报错
			xhr.send(formdata);
		},
		// 返回需要上传的文件
		...
		init : function(){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if(self.uploadInput){
				...
				this.uploadInput.addEventListener("click", function(e) {
					self.funUploadFiles(false);//不提交表单
				}, false);	
			}
		}
};
