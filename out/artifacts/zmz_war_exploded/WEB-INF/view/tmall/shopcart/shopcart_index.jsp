...
//			});
			...
			$("#btnPay").click(function(){
				var useVoucher=$("#useVoucher").is(":checked")?1:0;//选中为1  没有选中为0
                var useAdvance=$("#useAdvance").is(":checked")?1:0;//选中为1  没有选中为0
				var str=prompt("请填写备注");
				if(str==null||str==""){
					alert("备注信息必填");
				}else{
					$.ajax("<c:url value='/transfer/hf_tmallApplyFrom.json'/>",
							{"type":"POST","data":{"remittance":str,"useVoucher":useVoucher,"useAdvance":useAdvance},"success":function(xhr){
								var  m = xhr;
								if(m.flag=="0"){
									alert("申请成功！");
					...
					<%--$("#btn-transfer").click(function(){--%>
						...
						<%--var str=prompt("请与您的供货商联系付款并提醒对方确认收款,在此填写汇款账号信息");--%>
						 ...
						 <%--}else{--%>
							 		...
							 		<%--var useVoucher=$("#useVoucher").is(":checked")?1:0;//选中为1  没有选中为0--%>
									<%--$.ajax("<c:url value='/transfer/hf_tmallApplyFrom.json'/>",--%>
											<%--{"type":"POST","data":{"remittance":str,"useVoucher":useVoucher,"useAdvance":false},"success":function(xhr){--%>
												<%--var  m = xhr;--%>
												<%--if(m.flag=="0"){--%>
												<%--alert("申请成功！");--%>
											<%----%>
			...
			$("#products").delegate(".buyBtn", "click", function(e) {
				...
				$.ajax("<c:url value='/tmall/shopcart/add'/>", {
					"type" : "POST",
					"data" : {
						"goodsId" : id
					}
