								...
								<c:forEach items="${ts}" var="g">
									<tr data-row="${g.id}">
										...
										<a class="btn btn-success default ajaxLink" 
											...
											href="<c:url value='/transfer/confirm.json?id=${g.id}' /> "><span class="fa fa-share-square-o" aria-hidden="true"></span>同意转货</a>
											<a class="btn btn-warning default ajaxLink"
											<c:if test="${g.status!='NEW' }">disabled="disabled"</c:if>
											href="<c:url value='/transfer/refuse.json?id=${g.id}' /> "><span class="glyphicon glyphicon-minus-sign" aria-hidden="true"></span>拒绝转货</a>
											</c:if>
											<c:if test="${g.userByToAgent.id eq user.id }">
											<a class="btn btn-success default ajaxLink" data-role="DELETE"
											<c:if test="${g.status!='NEW'}">disabled="disabled"</c:if>
											href="<c:url value='/transfer/remove.json?id=${g.id}' /> "><span class="fa fa-share-square-o" aria-hidden="true"></span>撤销申请</a>
											</c:if>
											</c:if>
											<!--如果是退货-->
											<c:if test="${g.backedTransfer}">
											...
											<c:if test="${user.id eq 1 }">
												<a class="btn btn-success default ajaxLink"
												   <c:if test="${g.status!='NEW'}">disabled="disabled"</c:if>
												href="<c:url value='/transfer/confirm.json?id=${g.id}' /> "><span class="fa fa-share-square-o" aria-hidden="true"></span>同意退货</a>
												<a class="btn btn-warning default ajaxLink"
												   <c:if test="${g.status!='NEW' }">disabled="disabled"</c:if>
												   href="<c:url value='/transfer/refuse.json?id=${g.id}' /> "><span class="glyphicon glyphicon-minus-sign" aria-hidden="true"></span>拒绝退货</a>
											</c:if>
											</c:if>
										</td>
