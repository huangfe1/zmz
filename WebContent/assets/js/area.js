		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				...
				for(i=0;i<ar.length;i++){
					options[length]=new Option(ar[i],ar[i]);//后面的length+1
                    if(ar[i]==pcc[v]){options[length-1].selected=true};
				}//end for
              // alert(options[8].value);
...
function _init_area(){  //初始化函数
    ...
    for(i=0;i<s.length;i++){
        pcc[i]=document.getElementById(s[i]).getAttribute("val");//获取当前值
    }
	change(0);
}
